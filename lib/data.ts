...
export const defaults={
 ...
 heroTitleBefore:"创造",heroTitleAccent:"恰到好处",heroTitleAfter:"的声音。",headline:"游戏音频设计师，拥有从声音风格、音效资产到 Wwise 管线、Unity 集成与实机调试的独立游戏音频开发全流程经验。",heroPrimary:"浏览代表作品",heroSecondary:"播放 Showreel",
 reelTitle:"SOUND DESIGN REEL",reelEmpty:"作品视频即将上线",reelLabel:"SHOWREEL · 2026",
 workKicker:"SELECTED WORK",workTitle:"项目展示",workIntro:"声音设计、Wwise 实现与 Unity 联调，覆盖独立游戏音频制作的完整流程。",
 ...
 experienceKicker:"EXPERIENCE & CREDITS",experienceTitle:"实习经历",
 ...
 aboutKicker:"ABOUT",aboutTitle:"关于我",about:"我关注声音与玩家行为之间的关系，也享受把创意制作、技术实现和团队协作连成一条可靠的音频管线。",location:"BASED IN LONDON",
...
type AdminToken={exp:number};
function tokenSecret(){return process.env.ADMIN_SESSION_SECRET||""}
function sign(value:string){return createHmac("sha256",tokenSecret()).update(value).digest("base64url")}
export function createAdminToken(){const payload=Buffer.from(JSON.stringify({exp:Date.now()+12*60*60*1000} satisfies AdminToken)).toString("base64url");return `${payload}.${sign(payload)}`}
export function validateAdminToken(token:string){
 if(!tokenSecret()||!token.includes("."))return false;
 const[payload,signature]=token.split(".");const expected=sign(payload);
 if(signature.length!==expected.length||!timingSafeEqual(Buffer.from(signature),Buffer.from(expected)))return false;
 try{return Number((JSON.parse(Buffer.from(payload,"base64url").toString()) as AdminToken).exp)>Date.now()}catch{return false}
}
export async function verifyAdmin(request:Request){const token=request.headers.get("authorization")?.replace(/^Bearer\s+/i,"")||"";const client=cloudbaseAdmin();return client&&validateAdminToken(token)?{client}:null}

export async function getPublicData():Promise<SiteData>{try{const client=cloudbaseAdmin();if(!client)return{...defaults,items:[]};const[{data:settings},{data:items}]=await Promise.all([client.rdb().from("settings").select("key,value"),client.rdb().from("portfolio_items").select("id,title,category,description,media_key,media_type,file_name,placement").order("id",{ascending:false})]);const values=Object.fromEntries(((settings??[]) as Array<{key:string;value:string}>).map(x=>[x.key,x.value]));return{...defaults,...values,items:((items??[]) as Array<Record<string,any>>).map(x=>({id:x.id,title:x.title,category:x.category,description:x.description,mediaKey:x.media_key,mediaType:x.media_type,fileName:x.file_name,placement:x.placement||"archive"}))}}catch{return{...defaults,items:[]}}}
